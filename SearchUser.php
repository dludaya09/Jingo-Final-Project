...
<div id="Jingo">
    ...
    <form id="SearchUser" action="SearchUser.php" method="post">
    
    
     <h2> Search User </h2> <br>
    <label for="tagid">Enter Username:</label>
...
{?>
  ...
  alert("Please Login");
  window.close("http://localhost/jingo/SearchUser.php");
  window.open("http://localhost/jingo/Home.php");
  </script>
  <?php
}
$usr=$_SESSION['userid'];
...
$sql="Select userid from user where userid <> '$ID' and username like '%".$username."%'";
if($result=($mysqli->query($sql)))
{
  while ($row = $result->fetch_row()){
  foreach($row as $key=>$value){ ?><br><font size="4" face="century gothic">
  <a href="ViewUserProfile.php?val= <?php echo $value; ?>">
  
    <form id="User" action="SearchUser.php" method="post">
    <fieldset>
    <label for="userid">Userid: <?php echo $value; ?></label><br>
    </fieldset>
</form>
...
<script type="text/javascript">
function logout(){
  window.open("http://localhost/jingo/logout.php");
}
function home(){
  window.open("http://localhost/jingo/Search.php");
}
</script>
</form>
</font>
...
#SearchUser input, #Profile2 textarea{
display:block;
}
...
#SearchUser input#Submit:active{
border:1px solid #3084a4;
}

#SearchUser input#Submit:hover, #Login input#login:focus{
